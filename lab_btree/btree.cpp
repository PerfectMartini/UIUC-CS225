...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     * | 0 | 5 | 7 | 8 |
     * Suppose we are looking for 5. first_larger_idx is 2. This means we want to
     * explore the child between 4 and 7. The children vector has a pointer for
     * each of the horizontal bars. The index of the horizontal bar we want is
     * 1, which is conveniently the same as first_larger_idx. If the subroot is
